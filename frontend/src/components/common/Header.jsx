...
export const Header = ({ role: propRole, onSearchChange }) => {
  const { role: contextRole, creatorUser, editorUser } = useAuth();
  ...
  return (
    <header className="bg-[#07090E] border-b border-white/[0.06] sticky top-0 z-20">
      <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row md:items-center md:justify-between gap-4 py-5 px-6 md:px-8">
        {/* Welcome Title */}
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
            Welcome back, <span className={role === 'creator' ? 'text-purple-400' : 'text-blue-400'}>{role === 'creator' ? 'Creator!' : 'Editor!'}</span>
        ...
        <div className="flex items-center gap-3.5 flex-wrap">
          ...
          {role === 'creator' ? (
            ...
            <button
              onClick={handleCtaClick}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-all shadow-md shadow-blue-900/30 active:scale-[0.98]"
            >
          ...
          >
            <div className="relative">
              <img
                ...
                className={`w-9 h-9 rounded-full object-cover border transition-all ${
                  role === 'creator'
                    ? 'border-purple-500/40 ring-2 ring-purple-600/20 group-hover:ring-purple-500'
                    : 'border-blue-500/40 ring-2 ring-blue-600/20 group-hover:ring-blue-500'
                }`}
              />
            ...
            <div className="hidden xl:block text-left">
              ...
              <span className="text-[11px] text-slate-400 block">{role === 'creator' ? 'Creator' : 'Editor Pro'}</span>
            </div>
          </div>
        </div>
      </div>
